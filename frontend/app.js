...
async function geocodeAddress(address) {
  ...
  if (!data.results || data.results.length === 0) {
    throw new Error('Address not found. Try a more specific address, e.g. "123 Ang Mo Kio Ave 3".');
  }
...
function buildPayload(lat, lon) {
  ...
  if (isNaN(leaseStart)) throw new Error('Lease commencement year is required.');
  ...
  const floor_area_sqm = parseFloat(document.getElementById('floor_area').value);
  ...
  if (remaining_lease_years < 0 || remaining_lease_years > 99) {
    throw new Error('Remaining lease is out of range. Check lease commencement year.');
  }

  return {
    town, flat_type, flat_model, storey_midpoint, floor_area_sqm,
    remaining_lease_years, lease_commence_date: leaseStart,
    transaction_year: txYear, transaction_month: txMonthNum, lat, lon,
  };
}
...
function renderSHAP(shapValues) {
  ...
  const layout = {
    ...
    xaxis: {
      title: { text: 'SHAP value (SGD)', font: { size: 12, family: 'Inter', color: '#6B7A8D' } },
  ...
  Plotly.react('shap-chart', [trace], layout, { responsive: true, displayModeBar: false });
}
...
function formatFeatureName(name) {
  const map = {
    ...
    mrt_count_1km: 'MRT within 1km',
    mrt_count_2km: 'MRT within 2km',
    school_count_1km: 'Schools within 1km',
    mall_count_2km: 'Malls within 2km',
    hawker_count_500m: 'Hawkers within 500m',
  ...
  if (name.startsWith('town_')) return 'Town: ' + toTitleCase(name.replace('town_', '').replace(/_/g, ' '));
  if (name.startsWith('flat_model_')) return 'Model: ' + name.replace('flat_model_', '').replace(/_/g, ' ');
...
function displayResults(data, lat, lon, payload) {
  ...
  document.getElementById('result-interval').innerHTML =
    `80% confidence interval: <strong>SGD ${lower.toLocaleString('en-SG')}</strong> — <strong>SGD ${upper.toLocaleString('en-SG')}</strong>`;

  const warn = document.getElementById('crossing-warning');
  data.quantile_crossing ? warn.classList.remove('hidden') : warn.classList.add('hidden');
...
function syncSliders(payload) {
  ...
  set('slider-lease', 'val-lease', Math.round(payload.remaining_lease_years));

  if (lastPrediction && lastPrediction.feature_values) {
    const fv = lastPrediction.feature_values;
    set('slider-mrt', 'val-mrt', (fv.dist_nearest_mrt || 0.5).toFixed(2));
    set('slider-cbd', 'val-cbd', (fv.dist_to_cbd || 5).toFixed(1));
  }
}
...
function onSliderChange() {
  ...
  const mrt = parseFloat(document.getElementById('slider-mrt').value).toFixed(2);
  const cbd = parseFloat(document.getElementById('slider-cbd').value).toFixed(1);
...
const TOWN_MEDIANS = {
  'Queenstown': 785000, 'Bukit Merah': 745000, 'Bishan': 720000,
  'Toa Payoh': 700000, 'Marine Parade': 698000, 'Central Area': 690000,
  'Kallang/Whampoa': 680000, 'Clementi': 658000, 'Bukit Timah': 640000,
  'Serangoon': 610000, 'Ang Mo Kio': 598000, 'Geylang': 580000,
  'Tampines': 568000, 'Bedok': 558000, 'Hougang': 548000,
  'Pasir Ris': 540000, 'Jurong East': 532000, 'Punggol': 525000,
  'Sengkang': 518000, 'Bukit Panjang': 515000, 'Yishun': 505000,
  'Bukit Batok': 495000, 'Woodlands': 480000, 'Choa Chu Kang': 475000,
  'Jurong West': 468000, 'Sembawang': 455000,
};
...
const TREND_DATA = {
  ...
  values: [
    ...
    510000,565000,610000,645000,660000,665000,668000,672000,678000,685000,
  ],
};
...
function renderExplorerCharts() {
  const towns = Object.keys(TOWN_MEDIANS).sort((a, b) => TOWN_MEDIANS[b] - TOWN_MEDIANS[a]);
  const prices = towns.map((t) => TOWN_MEDIANS[t]);
  const maxPrice = Math.max(...prices);

  Plotly.newPlot('town-chart', [{
    type: 'bar',
    x: prices,
    y: towns,
    orientation: 'h',
    ...
    xaxis: {
      ...
      gridcolor: '#E2E8EF',
      tickformat: ',.0f',
      title: { text: 'Median resale price (SGD)', font: { size: 12, family: 'Inter', color: '#6B7A8D' } },
    },
    yaxis: {
      tickfont: { size: 12, family: 'Inter', color: '#0F1923' },
      automargin: true,
